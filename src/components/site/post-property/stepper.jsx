...
import { POST_PROPERTY_STEPS } from "@/schemas/site/postPropertySchema";
import { cn } from "@/lib/utils";

export function PostPropertyStepper({ currentIndex, furthestIndex, onStepClick }) {
  return (
      ...
      {POST_PROPERTY_STEPS.map((step, index) => {
        ...
        return (
          <button
            ...
            className={cn(
              "flex min-h-9 shrink-0 items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-medium transition-colors",
              active
          ...
          >
            <span
              className={cn(
                ...
                active ? "bg-primary-600 text-white" : completed ? "bg-success-600 text-white" : "bg-surface-muted text-foreground-muted"
              )}
            >
